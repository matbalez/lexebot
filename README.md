...
amount, plus a PNG QR code image. LexeBot first tries a Sprout media upload,
then falls back to an inline PNG data URL if upload is unavailable.

`fund wallet` returns a reusable no-minimum BOLT12 offer for funding the Lexe
wallet, plus the same PNG QR image behavior.
...
```bash
curl -L -o lexebot-v0.1.25-aarch64-apple-darwin.tar.gz \
  https://github.com/matbalez/lexebot/releases/download/v0.1.25/lexebot-v0.1.25-aarch64-apple-darwin.tar.gz
curl -L -o lexebot-v0.1.25-aarch64-apple-darwin.tar.gz.sha256 \
  https://github.com/matbalez/lexebot/releases/download/v0.1.25/lexebot-v0.1.25-aarch64-apple-darwin.tar.gz.sha256
shasum -a 256 -c lexebot-v0.1.25-aarch64-apple-darwin.tar.gz.sha256
mkdir -p ~/.local/bin
tar -xzf lexebot-v0.1.25-aarch64-apple-darwin.tar.gz
...
sets up LexeBot locally, opens a Direct Message with the local LexeBot identity
for manual wallet commands, and starts LexeBot:
...
₿2,500, it also sends a funding prompt with a reusable BOLT12 offer and PNG QR
code image. On a fresh install, it prints
...
subscriptions, the normal installer path opens/configures a LexeBot DM for
manual wallet commands.

If you do not want the installer to use or build Sprout CLI to open the LexeBot
DM, use manual-add mode:
...
mode; configure the LexeBot DM/control channel only if the owner should send
...
LEXEBOT_UPDATE_CHECK=1
```

`LEXEBOT_MAX_SEND_AMOUNT` is expressed in ₿ base units. If set, `send` commands
above that amount are rejected.

LexeBot checks the latest GitHub release on startup and every six hours while
running. If a newer release exists, it writes a one-time upgrade notice to the
owner LexeBot DM, or logs the notice if no owner DM is configured. Set
`LEXEBOT_UPDATE_CHECK=0` to disable outbound update checks.
...
terse public confirmation. The sender LexeBot also writes a receipt such as
`₿21 kudos sent to @DK` into the owner's configured LexeBot DM when that DM is
available. The sender LexeBot dedupes auto-kudos sends by the original
`source_event_id`.

Kudos also sends a NIP-44 encrypted `auto-kudos-receive-notice` to the receiver
LexeBot before asking the sender LexeBot to pay. The receiver LexeBot does not
poll forever; it only polls recent Lexe payments once per second while a receive
notice is pending, then stops when the matching `Kudos!` inbound payment is seen
or after a timeout. When the payment arrives, it writes a receiver DM receipt
such as `Received ₿21 kudos.`.

The Lexe payment message is `Kudos!`, and the sender's private personal note is
`You sent a Kudos to <name>`. The result event is only p-tagged to Kudos;
request correlation stays inside the encrypted payload. Failures are logged to
the LexeBot terminal.
Normal wallet commands still require the owner pubkey and still happen in a
configured channel/DM.
